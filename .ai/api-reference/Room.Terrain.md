...
```
Get terrain type at the specified room position by `(x,y)` coordinates. Unlike the [`Game.map.getTerrainAt(...)`](#Game.map.getTerrainAt) method, this one doesn't perform any string operations and returns integer terrain type values (see below).
| parameter | type | description |
| --- | --- | --- |
...
| value | [constant](#Constants) (*if exists*) | description |
| --- | --- | --- |
...
auto u8ptr = static_castuint8_t*>(ptr);
// computations here...
}
```
...
| --- | --- | --- |
| `destinationArray`
*optional* | Uint8Array | A typed array view in which terrain will be copied to. |
...
See usage examples. Learn more about [*binary modules*](/modules.html#Binary-modules).
