...
- `src/creeps/jobs/traffic.ts` — traffic-yield request/honor orchestration and yield-memory cleanup
- `src/creeps/jobs/trafficYieldHelpers.ts` — traffic blocker priority checks and yield-position selection helpers
...
- `src/rooms/energy.ts` — room energy pressure/recovery hysteresis orchestration and stored-energy helpers
- `src/rooms/energyRefill.ts` — refill constants plus spawn/tower/terminal refill target helpers
...
- `src/rooms/jobs/assignmentFlow.ts` — local energy-carrying and post-energy fallback assignment flow helpers
...
- `src/rooms/jobs/energyTargets.ts` — local energy gather/withdraw target-selection orchestration
- `src/rooms/jobs/energyDeposit.ts` — local energy deposit target selection policy for haulers/workers
- `src/rooms/jobs/energyWork.ts` — worker/hauler energy spending orchestration
- `src/rooms/jobs/energyWorkAssignment.ts` — build/repair/upgrade/refill assignment helpers for energy spending
- `src/rooms/jobs/energyWorkPrimaryResume.ts` — primary build/repair/upgrade resume checks and reservation carry-forward
...
- `src/rooms/jobs/validity.ts` — current-job validity orchestration and non-transfer job guards
- `src/rooms/jobs/validityTransfer.ts` — withdraw/pickup/deposit job validity helpers
...
- `src/rooms/spawning/requestDemand.ts` — local spawn-demand thresholds and worker/hauler demand math
...
- `src/rooms/remotes/energy.ts` — remote hauler energy-target orchestration and target-path gating
- `src/rooms/remotes/energyFallback.ts` — remote fallback dropped/container/link target selection and stuck-target avoidance helper
...
- Energy pressure and recovery hysteresis — `src/rooms/energy.ts`; refill constants and spawn/tower/terminal refill target helpers in `src/rooms/energyRefill.ts`; terminal reserve floors and withdrawability/deficit helpers in `src/rooms/energyTerminal.ts`
- Job memory writes and primary-job resume memory — `src/creeps/jobs/memory.ts`
- Local source/mineral/gather/fallback job assignment — orchestration in `src/rooms/jobs/assignment.ts`, energy-carrying/post-energy flow helpers in `src/rooms/jobs/assignmentFlow.ts`, and haul/resource acquisition policy in `src/rooms/jobs/assignmentHauling.ts`
...
- Current-job target validity and resource/work guards — orchestration/non-transfer checks in `src/rooms/jobs/validity.ts`, withdraw/pickup/deposit helpers in `src/rooms/jobs/validityTransfer.ts`
- Energy spending and primary build/repair/upgrade resume — energy-spending orchestration in `src/rooms/jobs/energyWork.ts`, assignment helpers in `src/rooms/jobs/energyWorkAssignment.ts`, and primary-job resume checks/reservations in `src/rooms/jobs/energyWorkPrimaryResume.ts`
- Local energy gather/withdraw target selection — `src/rooms/jobs/energyTargets.ts`; deposit target selection in `src/rooms/jobs/energyDeposit.ts`; mining-site pickup thresholds and container-id mining-site helpers in `src/rooms/jobs/energyMiningSites.ts`
...
- Local spawn demand selection — demand-threshold helpers in `src/rooms/spawning/requestDemand.ts`, request modeling/selection in `src/rooms/spawning/requestSelection.ts`, spawn loop orchestration in `src/rooms/spawning/planner.ts`, spawn attempt/budget/scale/minimum helpers in `src/rooms/spawning/plannerSpawnAttempt.ts`, and pending-spawn snapshot plus spawn name/memory helpers in `src/rooms/spawning/spawnRequestHelpers.ts`. Multiple free spawns share a pending-request ledger with planned bodies so in-flight creeps count toward capacity and per-source/per-role caps via `src/rooms/spawning/accounting.ts`.
