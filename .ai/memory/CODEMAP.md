...
- Remote hauler cycle (dropped-first remote pickup, fill-to-full top-up, far-pickup return at >=75% load, home storage deposit, post-trip renew only when TTL<1000 to TTL>1400, no-job home idle/wander with TTL<500 renew gate) — `src/room.controller.ts`; renew requests reserve free spawns through `src/spawn.renewal.ts` so multiple renewers spread across multiple spawns.
