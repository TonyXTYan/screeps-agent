...
- (Fixed, local-only threshold) Local haulers no longer make low-value mining-site trips: mining-site pickups for `hauler` remain gated at 50% carry capacity. Remote haulers now use dropped-first fill/top-up flow instead of a hard 50% mining-site threshold.
...
- (Fixed) Remote hauler cycle drift: remote haulers previously mixed with generic remote flow (no dedicated post-delivery renew phase, could idle in remotes when empty, and could deliver to non-storage sinks). Remote haulers now use a dedicated cycle: dropped-first remote pickup with fill/top-up behavior, home storage deposit, conditional post-trip renew (only when TTL < 1000, to TTL > 1400), and no-job home idle/wander (>=6 range from spawn) with renew-on-ttl<500 behavior.
