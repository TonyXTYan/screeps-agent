...
| **Patrol** | Dynamic (`RCL >= 6`) | `ceil(enabledRemotes / 2) + visibleArmedHostiles` |
...
- `dangerUntil` no longer blocks normal remote assignment/spawn flow by default.
- Danger marker escalation is fail-safe only (zero patrol coverage + visible armed hostiles).
