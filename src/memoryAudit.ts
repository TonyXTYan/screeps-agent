import {
    cleanupInvalidCreepMemory,
    fixDuplicateSourceAssignments,
    fixOrphanedSourceReferences,
    fixStaleTravelMemory
} from './memoryAuditCreeps';
import {
    cleanupOrphanedRoomMemory,
    cleanupStaleRemotePlans
} from './memoryAuditRooms';

export function runFullAudit(): number {
    let fixed = 0;

    const activeRooms = collectActiveRooms();
...
function collectActiveRooms(): Set<string> {
