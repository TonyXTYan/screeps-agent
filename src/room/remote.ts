import { getCreepCapabilities, ensureArchetype, getBodyCapabilities, planBodyForArchetype } from "../creep.capabilities";
...
import { REMOTE_MINER_OSCILLATION_REPLAN_TICKS, REMOTE_MINER_NO_PROGRESS_REPLAN_TICKS, REMOTE_MINER_STUCK_REPLAN_TICKS, REMOTE_MAX_STATION_STALLS, REMOTE_MAX_STATION_FAILURES, REMOTE_STANDBY_PARK_RANGE_MAX, REMOTE_STANDBY_PARK_RANGE_MIN, REMOTE_STANDBY_BOUNDARY_STUCK_TICKS, REMOTE_STANDBY_PARK_RANGE_TARGET, REMOTE_HAULER_FAR_PICKUP_PATH_LENGTH, REMOTE_HAULER_FAR_PICKUP_RETURN_LOAD_RATIO, REMOTE_HAULER_POST_TRIP_RENEW_START_TTL, REMOTE_HAULER_RENEW_STOP_TTL, REMOTE_HAULER_IDLE_RECHECK_TICKS, REMOTE_HAULER_RENEW_START_TTL, REMOTE_HAULER_RENEW_CRITICAL_TTL, REMOTE_HAULER_WANDER_MIN_RANGE, REMOTE_HAULER_WANDER_MAX_RANGE, REMOTE_HAULER_WANDER_TICKS, TOWER_RECOVERY_RATIO, REMOTE_DANGER_TICKS, REMOTE_CONTAINER_REROUTE_FREEZE_TICKS, REMOTE_CONTAINER_BUILD_DISTANCE, REMOTE_PATH_REFRESH_INTERVAL, REMOTE_PATH_INCOMPLETE_RETRY_TICKS, REMOTE_INACCESSIBLE_RETRY_TICKS, REMOTE_PLANNING_LOG_INTERVAL, MAX_REMOTE_HAULER_CAPACITY_PER_SOURCE, REMOTE_ROAD_SITES_PER_TICK, REMOTE_DEGRADED_MAX_UNFINISHED_ROAD_SITES, REMOTE_MAX_UNFINISHED_ROAD_SITES, REMOTE_RENEW_MIN_TTL, REMOTE_RENEW_BUFFER_TICKS, REMOTE_RENEW_HYSTERESIS, REMOTE_AUX_BUILD_RANGE, REMOTE_STANDBY_TRIGGER_TTL, REMOTE_SCOUT_CROWD_THRESHOLD, REMOTE_SCOUT_WANDER_TICKS, REMOTE_REPLACEMENT_BUFFER_TICKS, REMOTE_HAULER_ASSIGNED_SOURCE_MIN_ENERGY, REMOTE_HAULER_CROSS_SOURCE_MIN_ENERGY, REMOTE_HAULER_RETARGET_STUCK_TICKS, REMOTE_TARGET_MAX_HAULER_CLAIMS } from "./constants";
import { setTravelJob, setJob, closest, firstStoredResource, towerEnergyRatio, setResourceJob, closestByRange } from "./jobs";
import { creepsForHomeRoom, sourceWorkDemand } from "./spawn";
import { RoomControllerContext, RemoteEnergySourceTarget } from "./types";
...
export function remoteMinerStationRouteStalled(creep: Creep, source: Source, sourcePlan: RemoteSourcePlan): boolean {
...
export function preferredRemoteInfrastructureSite(creep: Creep, archetype: CreepArchetype, selectedSite: ConstructionSite | null): ConstructionSite | null {
...
export function assignRemoteHaulerCycle(creep: Creep, homeRoom: string, remoteRoom: string, remotePlan: RemoteRoomPlan): boolean {
...
export function shouldDeferRemoteHaulerRenewal(homeRoom: Room, ttl: number): boolean {
...
export function remoteHaulerWanderTarget(creep: Creep, spawn: StructureSpawn): RoomPosition {
...
export function clampRoomCoord(value: number): number {
...
export function placeRemoteRoadSites(homeRoom: Room, visibleRemote: Room, sourcePlan: RemoteSourcePlan, latestPath: RoomPosition[], myUsername: string | undefined, siteLimit: number, maxToPlace: number, unfinishedRoadSites: number): number {
    let placed = 0;
...
export function canPlaceRemoteRoadSite(homeRoom: Room, visibleRemote: Room, pos: RoomPosition, myUsername: string | undefined): boolean {
...
export function shouldBuildRemoteInfrastructure(creep: Creep, archetype: CreepArchetype, remotePlan: RemoteRoomPlan): boolean {
...
export function manageRemoteRenewal(creep: Creep, archetype: CreepArchetype, capabilities: ReturnType<typeof getCreepCapabilities>, homeRoomName: string, remoteRoomName: string, remotePlan: RemoteRoomPlan): boolean {
    const ttl = creep.ticksToLive;
    if (!ttl) { return false; }
    ...
    if (capabilities.claim > 0) { return false; }
...
export function estimateRemoteDistance(creep: Creep, homeRoomName: string, remoteRoomName: string, remotePlan: RemoteRoomPlan): number {
...
export function sourceNeedingBlankStandbyMiner(creeps: Creep[], remoteRoom: string, remotePlan: RemoteRoomPlan, excludeCreepId?: string): string | null {
...
export function findDyingRemoteMiner(creeps: Creep[], remoteRoom: string): Creep | null {
...
export function hasActiveRemoteMinerForSource(creeps: Creep[], remoteRoom: string, sourceId: string, excludeCreepId?: string): boolean {
    for (const creep of creeps) {
        if (creep.id === excludeCreepId) { continue; }
        if (creep.spawning) { continue; }
        if (ensureArchetype(creep) !== 'remoteMiner') { continue; }
...
export function sourceNeedingStandbyReplacement(creeps: Creep[], remoteRoom: string): string | null {
...
export function pickRemoteMinerSource(creep: Creep, sources: Source[], remotePlan: RemoteRoomPlan, minerCountBySource: Map<string, number>): Source | null {
...
export function assignOverflowRemoteScout(creep: Creep, homeRoomName: string, wanderFallbackRoom: string = homeRoomName): boolean {
...
export function projectedRemoteMinerWork(creeps: Creep[], remoteRoom: string, sourceId: string, horizonTicks: number): number {
    let total = 0;
    for (const creep of creeps) {
        if (ensureArchetype(creep) !== 'remoteMiner') { continue; }
        if (creep.memory.remoteRoom !== remoteRoom) { continue; }
...
export function projectedRemoteHaulerCapacity(creeps: Creep[], remoteRoom: string, sourceId: string, horizonTicks: number): number {
    let total = 0;
    for (const creep of creeps) {
        if (ensureArchetype(creep) !== 'remoteHauler') { continue; }
        if (creep.memory.remoteRoom !== remoteRoom) { continue; }
...
export function remoteSourceReplacementHorizon(context: RoomControllerContext, sourcePlan: RemoteSourcePlan, archetype: 'remoteMiner' | 'remoteHauler'): number {
...
export function findRemoteEnergySource(creep: Creep, remotePlan: RemoteRoomPlan, opts?: { followDroppedTopUp?: boolean; droppedFirst?: boolean; droppedMinAmount?: number }): RemoteEnergySourceTarget | null {
    const avoidTargetId = remoteHaulerAvoidTargetId(creep);
...
export function bestRemoteEnergyTargetForSource(creep: Creep, remotePlan: RemoteRoomPlan, sourceId: string, avoidTargetId: string | undefined, droppedMinAmount: number, skipDroppedResources: boolean): RemoteEnergySourceTarget | null {
...
export function bestCrossSourceRemoteEnergyTarget(creep: Creep, remotePlan: RemoteRoomPlan, assignedSourceId: string, avoidTargetId: string | undefined, minEnergy: number): RemoteEnergySourceTarget | null {
...
export function remoteEnergyAvailableForSource(creep: Creep, remotePlan: RemoteRoomPlan, sourceId: string, droppedMinAmount: number): number {
...
export function remoteEnergyTargetsForSource(creep: Creep, remotePlan: RemoteRoomPlan, sourceId: string, droppedMinAmount: number, skipDroppedResources: boolean): RemoteEnergySourceTarget[] {
    const cfg = remotePlan.sources?.[sourceId];
    if (!cfg) { return []; }
...
export function pickRemoteEnergySourceTarget(creep: Creep, targets: RemoteEnergySourceTarget[]): RemoteEnergySourceTarget | null {
...
export function remoteEnergyTargetPathLength(creep: Creep, target: RoomObject & { id: string }): number | null {
...
export function pickRemoteEnergyTarget<T extends RoomObject & { id: string }>(creep: Creep, targets: T[], avoidTargetId?: string): T | null {
    if (targets.length === 0) { return null; }
...
export function shouldAvoidRemoteEnergyTarget(creep: Creep, target: RoomObject & { id: string }, avoidTargetId?: string): boolean {
...
export function isRemoteEnergyTargetReachable(creep: Creep, target: RoomObject & { id: string }): boolean {
...
export function remoteEnergyAvailableAfterClaims(creep: Creep, target: StructureContainer | StructureLink | Resource<RESOURCE_ENERGY>): number {
